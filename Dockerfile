...
FROM scratch
COPY --from=0 /usr/local/lib/libopentracing.so.1.5.1 /root/libopentracing.so
COPY --from=0 /usr/local/lib/libzipkin.so.0.5.2 /root/libzipkin.so
COPY --from=0 /usr/local/lib/libzipkin_opentracing.so.0.5.2 /root/libzipkin_opentracing_plugin.so
COPY --from=0 /nginx/objs/ngx_http_opentracing_module.so /root/ngx_http_opentracing_module.so
COPY root/ /
